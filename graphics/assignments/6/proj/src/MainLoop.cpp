...
//    array<float, 18> lightSourceColor{};
//    lightSourceColor.fill(1);
//    ColoredCube source(lightSourceColor, 0.5, 0.5, 0.5);
...
//        Utils::printVec3(glm::vec3(sourceMovement.getPosition(), 0));
        cube.draw(&rotate);

        {
            if (ImGui::BeginMainMenuBar()) {
                if (ImGui::BeginMenu("Mode")) {
                    if (!cube.isUsingPhong() && ImGui::MenuItem("PHONG")) {
                        cube.setUsingPhong(true);
                    } else if (cube.isUsingPhong() && ImGui::MenuItem("GOUROUD")) {
                        cube.setUsingPhong(false);
                    }
