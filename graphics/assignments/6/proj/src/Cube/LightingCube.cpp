...
std::unique_ptr<Shader> LightingCube::phongShader = nullptr;
std::unique_ptr<Shader> LightingCube::gouraudShader = nullptr;
...
LightingCube::LightingCube(glm::vec3 colors, float length, float width, float height) {
    ...
    if (!phongShader) {
        phongShader = make_unique<Shader>(phong_vert_shader_path, phong_frag_shader_path);
    }
    if (!gouraudShader) {
        gouraudShader = make_unique<Shader>(gouroud_vert_shader_path, gouroud_frag_shader_path);
    }
...
bool LightingCube::isUsingPhong() const {
    return usingPhong;
}

void LightingCube::setUsingPhong(bool usingPhong) {
    LightingCube::usingPhong = usingPhong;
}
