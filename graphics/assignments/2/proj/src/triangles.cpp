...
array<float, 18> simple_vertices = {
        0.7f, -0.7f, 0.0f, 0.56f, 0.72f, 0.33f,
        -0.7f, -0.7f, 0.0f, 0.56f, 0.72f, 0.33f,
        0.0f, 0.7f, 0.0f, 0.56f, 0.72f, 0.33f,
};
...
array<unsigned int, 12> tri_elements_indices = {
        1, 3, 5,
        2, 3, 4,
        0, 4, 5,
};
array<unsigned int, 14> line_elements_indices = {
        1, 3, 3, 2,
        2, 4, 4, 0,
        3, 4, 4, 1,
        3, 0
...
pair<GLuint, GLuint> genVertex(int size, float *data, const function<void()> &vertexAttribCallback) {
    GLuint VBO, VAO;
...
void triangles::loop() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (mode == Mode::RASTERIZED_TRIANGLE) {
            ...
            glBufferData(GL_ARRAY_BUFFER, sizeof(rgb_vertices), rgb_vertices.data(), GL_STATIC_DRAW);
            ...
            int colorLocation = glGetUniformLocation(uniformShader->getProgram(), "uniColor");
            glUniform4f(colorLocation, customColor[0], customColor[1], customColor[2], 1.0f);
            glDrawArrays(GL_TRIANGLES, 0, 3);
...
//            glDrawArrays(GL_LINE_LOOP, 0, 3);
        ...
        auto waitTime = static_cast<long long>(1000.0 / 60 - double(clock() - start) / CLOCKS_PER_SEC * 1000);
        //Limit fps
...
triangles::triangles(GLFWwindow *window) : window(window) {
    ...
    tie(RGB_VAO, RGB_VBO) = genVertex(sizeof(rgb_vertices), rgb_vertices.data(), vertexAttributeSetter);
    tie(simple_VAO, simple_VBO) = genVertex(sizeof(simple_vertices), simple_vertices.data(), vertexAttributeSetter);
    tie(custom_VAO, custom_VBO) = genVertex(sizeof(custom_vertices), custom_vertices.data(), uniformAttributeSetter);
    tie(element_VAO, element_VBO) = genVertex(sizeof(elements_vertices), elements_vertices.data(), vertexAttributeSetter);

    tri_EBO = genEBO(element_VAO, sizeof(tri_elements_indices), tri_elements_indices.data());
    line_EBO = genEBO(element_VAO, sizeof(line_elements_indices), line_elements_indices.data());

    shader = std::make_unique<Shader>("shaders/vert.glsl", "shaders/frag.glsl");
    uniformShader = std::make_unique<Shader>("shaders/vert.glsl", "shaders/uniform_frag.glsl");
